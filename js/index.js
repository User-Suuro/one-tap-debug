...
let host =
  env === "local" ? "http://127.0.0.1:8000" : "https://otaa.wearehonu.com";
...
let getUser = function () {
  return localStorage.getItem("authUser" + version)
    ? JSON.parse(localStorage.getItem("authUser" + version))
    : null;
};
let getSelectedSubAccount = function () {
  let authUser = getUser();
  let selectedSubAccount = localStorage.getItem("selectedSubAccount" + version)
    ? JSON.parse(localStorage.getItem("selectedSubAccount" + version))
    : null;
  ...
  localStorage.setItem(
    ...
    JSON.stringify(selectedSubAccount)
  );
...
let loadHomePage = async function () {
  clockAudio =
    env === "prod"
      ? new Media(host + "/img/clock.mp3")
      : new Audio("audio/clock.mp3");
  notificationAudio =
    env === "prod"
      ? new Media(host + "/img/notification.wav")
      : new Audio("audio/notification.wav");
  ...
  if (authUser) {
    ...
    $$("#person-to-contact-contact-number").val(
      authUser.person_to_contact_contact_number
    );
    ...
    if (selectedSubAccount.medicalRecords.length === 0) {
      ...
      for (let i = 0; i < medicalRecords.length; i++) {
        content += "    <li>";
        content +=
          '        <a href="#" class="item-link item-content view-medical-record" data-medical-record-id="' +
          medicalRecords[i].id +
          '">';
    ...
    .create({
      ...
      buttons: [
        ...
        },
        {
          ...
          onClick: function (dialog, e) {
            let content =
              ' <div class="list no-hairlines-md" style="margin:20px 0 0 0">';
            content += "        <ul>";
            content += '            <li class="item-content item-input">';
            content += '                <div class="item-inner">';
            content +=
              '                    <div class="item-title item-label">Health Info / Condition</div>';
            content += '                    <div class="item-input-wrap">';
            content +=
              '                        <input type="text" name="edit_title" placeholder="e.g. Diabetes" value="' +
              title +
              '" required/>';
            content +=
            ...
            content += '                <div class="item-inner">';
            content +=
              '                    <div class="item-title item-label">Details (Optional)</div>';
            content += '                    <div class="item-input-wrap">';
            content +=
              '                        <textarea name="edit_details" class="resizable" placeholder="Add more details of this condition">' +
              details +
              "</textarea>";
            content +=
              '                        <span class="input-clear-button"></span>';
                  ...
                  onClick: function (dialog, e) {
                    ...
                    app.request({
                      ...
                      success: function (response, status, xhr) {
                        ...
                        app.dialog.close();
                        app.dialog.alert(
                          "Medical record successfully saved.",
                          "Success"
                        );
...
let loadSubAccountsPage = async function () {
  ...
  if (authUser) {
    ...
    for (let i = 0; i < authUser.subAccounts.length; i++) {
      ...
      content +=
        '            <input type="radio" name="sub_accounts" value="' +
        authUser.subAccounts[i].id +
        '" ' +
        (authUser.subAccounts[i].id === selectedSubAccount.id
          ? "checked"
          : "") +
        ">";
      ...
      content += '            <div class="item-inner">';
      content +=
        '                <div class="item-title" id="sub-account-name">' +
        authUser.subAccounts[i].name +
        "</div>";
      content += "            </div>";
      content += "        </label>";
      content += "    </li>";
  ...
  setTimeout(function () {
    ...
    for (let i = 0; i < authUser.subAccounts.length; i++) {
      if (authUser.subAccounts[i].id === parseInt(subAccountId)) {
        localStorage.setItem(
          "selectedSubAccount" + version,
          JSON.stringify(authUser.subAccounts[i])
        );
        loadHomePage();
  ...
  try {
    ...
    for (let i = 0; i < responders.length; i++) {
      ...
      markers[i] = new google.maps.Marker({
        ...
        icon:
          responders[i].type === "Human"
            ? getMapAssets().markerHuman
            : getMapAssets().markerVeterinary,
      });
    ...
    success: function (response, status, xhr) {
      ...
      if (!alert) {
        if (!clockAudio) {
          clockAudio =
            env === "prod"
              ? new Media(host + "/img/clock.mp3", loadAlertPageContent())
              : new Audio("audio/clock.mp3", loadAlertPageContent());
        } else {
...
let loadAlertPageContent = function () {
  ...
  let content =
    ' <div class="width-100" style="position:absolute; top:56px; left:0">';
  content +=
    '        <img src="img/ambulance-lights-2.png" style="width:130px" />';
  content += "    </div>";
  content +=
    '    <div class="width-100" id="ambulance-light" style="position:absolute; top:56px; left:0; transition:1s">';
  content +=
    '        <img src="img/ambulance-lights.png" style="width:130px" />';
  content += "    </div>";
  content +=
    '    <div class="width-100" id="remaining-seconds" style="position:absolute; top:94px; left:0; font-size:2.6em; font-weight:bold">8</div>';
...
let loadEmergencyPage = function () {
  try {
    emergencyMap = new google.maps.Map(
      document.getElementById("map-emergency"),
      {
        zoom: 17,
...
let loadAlert = function () {
  ...
  formData.append(
    "alert_id",
    authUser.responder ? loadedAlertId : getOngoingAlert().id
  );
...
let updateEmergencyMap = function (alert) {
  ...
  if (authUser.responder) {
    ...
    content +=
      '        <i class="f7-icons">' +
      $$(
        ".view-emergency[data-alert-id='" + loadedAlertId + "'] .f7-icons"
      ).html() +
      "</i>";
    content += "    </div>";
    content += '    <div class="item-inner">';
    ...
    content +=
      "                <div>" +
      $$(
        ".view-emergency[data-alert-id='" +
          loadedAlertId +
          "'] .alert-sub-account-name"
      ).html() +
      "</div>";
    content +=
      '                <div class="item-footer">' +
      $$(
        ".view-emergency[data-alert-id='" + loadedAlertId + "'] .item-footer"
      ).html() +
      "</div>";
    content += "            </div>";
    ...
    for (let i = 0; i < alertResponders.length; i++) {
      ...
      if (
        alertResponders[i].responder_id === authUser.responder.id &&
        alertResponders[i].status === "Responding"
      ) {
        content +=
          '<div class="width-100" style="margin-right:10px"><button class="button button-outline button-round" id="stop-response">Stop Response</button></div>';
        content +=
          '<div class="width-100"><button class="button button-fill button-round" id="complete-response">Set as Done</button></div>';
      } else {
        content +=
          '<button class="button button-fill button-round" id="respond">Accept and Respond</button>';
      }
          ...
          directionsService.route(
            {
              origin: new google.maps.LatLng(
                parseFloat(alertResponders[i].latitude),
                parseFloat(alertResponders[i].longitude)
              ),
  ...
  if (authUser.responder) {
    ...
    $$("#person-to-contact").attr(
      "data-sub-account-name",
      alert.subAccount.name
    );
    $$("#person-to-contact").attr(
      "data-responder-name",
      authUser.responder.name
    ...
    $$("#person-to-contact").attr(
      "data-responder-latitude",
      authUser.responder.latitude
    );
    $$("#person-to-contact").attr(
      "data-responder-longitude",
      authUser.responder.longitude
    );
    ...
    $$("#person-to-contact #contact-number").html(
      alert.user.person_to_contact_contact_number
    );
  }
  ...
  app.dialog
    .create({
      ...
      text:
        ...
        "" +
        " to inform him or her about this emergency.",
      buttons: [
        {
          text: "Cancel",
        },
        {
          ...
          onClick: function (dialog, e) {
            ...
            let subAccountName = $$("#person-to-contact").attr(
              "data-sub-account-name"
            );
            let responderName = $$("#person-to-contact").attr(
              "data-responder-name"
            );
            let responderLatitude = $$("#person-to-contact").attr(
              "data-responder-latitude"
            );
            let responderLongitude = $$("#person-to-contact").attr(
              "data-responder-longitude"
            );
            ...
            app.request({
              ...
              success: function (response, status, xhr) {
                app.dialog.close();
                app.dialog.alert(
                  "An email notification has been sent to " + name + ".",
                  "Email Sent"
                );
              },
              error: function (xhr, status) {
  ...
  window.plugins.webintent.startActivity(
    {
      ...
      url:
        ...
        "?&body=Hello " +
        name +
        ", I'm sorry to inform you that an emergency has happened to " +
        subAccountName +
        ". He/She is now being transported to " +
        responderName +
        ".",
    },
    function () {},
...
let loadAlerts = function () {
  ...
  app.request({
    ...
    success: function (response, status, xhr) {
      ...
      let mutedAlerts = localStorage.getItem("mutedAlerts" + version)
        ? JSON.parse(localStorage.getItem("mutedAlerts" + version))
        : [];
      ...
      for (let i = 0; i < alerts.length; i++) {
        if (alerts[i].status === "Ongoing") {
          ...
          content +=
            '            <div class="item-media view-emergency" data-alert-id="' +
            alerts[i].id +
            '">';
          content +=
            '                <i class="f7-icons">' +
            (alerts[i].type === "Human" ? "person_alt" : "paw") +
            "</i>";
          content += "            </div>";
          content += '            <div class="item-inner">';
          content +=
            '                <div class="item-title view-emergency" data-alert-id="' +
            alerts[i].id +
            '">';
          content += '                    <div class="item-header">Name</div>';
          content +=
            '                    <div class="alert-sub-account-name">' +
            alerts[i].name +
            "</div>";
          content +=
            ...
            alerts[i].firstname +
            " " +
            (alerts[i].middlename ? alerts[i].middlename + " " : "") +
            alerts[i].lastname +
            "</div>";
          content += "                </div>";
          ...
          content += "                    <div>";
          content +=
            '                        <input type="checkbox" name="mute-alert" class="display-none" data-alert-id="' +
            alerts[i].id +
            '"' +
            (!mutedAlerts.includes(alerts[i].id) ? "checked" : "") +
            ">";
          content +=
            '                        <i class="f7-icons mute-alert unmuted ' +
            (mutedAlerts.includes(alerts[i].id) ? "display-none" : "") +
            '">speaker_3_fill</i>';
          content +=
            '                        <i class="f7-icons mute-alert muted ' +
            (!mutedAlerts.includes(alerts[i].id) ? "display-none" : "") +
            '">speaker_slash_fill</i>';
          content += "                    </div>";
          content += "                </div>";
          content += "            </div>";
          content += "        </div>";
    ...
    success: function (response, status, xhr) {
      ...
      for (let i = 0; i < alerts.length; i++) {
        if (alerts[i].status === "Completed") {
          content += "    <li>";
          content +=
            '        <div class="item-content view-history-alert" data-alert-id="' +
            alerts[i].id +
            '">';
          content += '            <div class="item-media">';
          content +=
          ...
          content +=
            ...
            "</div>";
          content +=
            '                    <div class="item-footer">Responder: ' +
            alerts[i].responder_name +
            "</div>";
          content +=
            '                    <div class="item-footer">Duration: ' +
            alerts[i].duration +
            "</div>";
          content += "                </div>";
          content += "            </div>";
...
$$(document).on("click", "#respond", function () {
  ...
  app.request({
    ...
    success: function (response, status, xhr) {
      ...
      let content =
        ' <button class="button button-outline button-round" style="margin-right:10px" id="stop-response">Stop Response</button>';
      content +=
        '    <button class="button button-fill button-round" id="complete-response">Set as Done</button>';
...
$$(document).on("click", "#stop-response", function () {
  ...
  app.request({
    ...
    success: function (response, status, xhr) {
      ...
      let content =
        '<button class="col button button-fill button-round" id="respond">Accept and Respond</button>';
          ...
          onClick: function (dialog, e) {
            ...
            app.request({
              ...
              success: function (response, status, xhr) {
                ...
                app.dialog.close();
                app.dialog.alert(
                  "Response is now completed and recorded.",
                  "Response Completed",
                  function () {
                    view.router.back();
                  }
